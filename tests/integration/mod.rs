...
use brea_scrapers::{ArgenPropScraper, Scraper, ScraperType};
use std::path::Path;
...
async fn test_single_property_scraping() {
    ...
    for (property, _images) in properties {
        db.save_property(&property).unwrap();
    }
...
async fn test_multiple_properties_scraping() {
    ...
    for (property, _images) in all_properties {
        db.save_property(&property).unwrap();
    }
...
async fn test_property_update_detection() {
    ...
    for (property, _images) in &initial_properties {
        db.save_property(property).unwrap();
    }
    ...
    for (property, _images) in &updated_properties {
        db.save_property(property).unwrap();
    }
    ...
    for property in saved_properties {
        let price_history = db.get_price_history(&property.id.unwrap()).unwrap();
